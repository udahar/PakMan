...
class InstitutionalSignals:
    ...
    def calculate_relative_strength(
        ...
        stock_return = (df['Close'].iloc[-1] - df['Close'].iloc[-period]) / df['Close'].iloc[-period]
        
        # Calculate benchmark return
        benchmark_return = (benchmark_df['Close'].iloc[-1] - benchmark_df['Close'].iloc[-period]) / benchmark_df['Close'].iloc[-period]
        ...
        if relative_strength > 0.10:  # Outperforming by >10%
            ...
            action = "WATCH"
            interpretation = f"Moderate relative strength: +{relative_strength:.1%} vs market"
            strength = min(100, relative_strength * 400)
        elif relative_strength < -0.10:  # Underperforming by >10%
            action = "SELL"
            interpretation = f"Weak relative strength: {relative_strength:.1%} vs market"
            strength = min(100, abs(relative_strength) * 500)
        else:
            action = "WATCH"
            interpretation = f"Neutral relative strength: {relative_strength:.1%} vs market"
            strength = 50
    ...
    def calculate_volatility_contraction(
        ...
        if range_trend < 0 and volume_trend > 0:
            ...
            avg_range_old = np.mean(df_copy['range_pct'].iloc[-lookback*2:-lookback].values)
            
            contraction_ratio = avg_range_recent / avg_range_old if avg_range_old > 0 else 1
    ...
    def rank_stocks_by_score(
        self,
        all_signals: Dict[str, List[InstitutionalSignal]]
        ...
        for symbol, signals in all_signals.items():
            ...
            for signal in signals:
                ...
                if signal.signal_type == 'volume_shock':
                    ...
                    print(f"     Contraction: {signal.details['contraction_ratio']:.1f}x")
