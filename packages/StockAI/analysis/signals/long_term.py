...
class LongTermAnalyzer:
    ...
    def calculate_growth_score(self, info: Dict, df: pd.DataFrame) -> float:
        ...
        if len(df) >= 126:  # ~6 months
            momentum_6m = (df['Close'].iloc[-1] - df['Close'].iloc[-126]) / df['Close'].iloc[-126]
            if momentum_6m > 0.20:
                score += 15
    ...
    def analyze_stock(self, symbol: str, df: pd.DataFrame, info: Dict) -> LongTermRating:
        """
        ...
        high_52w = info.get('fiftyTwoWeekHigh', df['High'].max()) if len(df) >= 252 else df['High'].max()
        low_52w = info.get('fiftyTwoWeekLow', df['Low'].min()) if len(df) >= 252 else df['Low'].min()
        ...
        fcf_yield = info.get('freeCashflow', 0) / info.get('marketCap', 1) if info.get('freeCashflow') and info.get('marketCap') else 0
        ...
        if growth_score >= 70:
            reasoning.append(f"Strong growth: {revenue_growth:.1%} revenue, {eps_growth:.1%} EPS")
        elif growth_score < 40:
            reasoning.append(f"Weak growth: {revenue_growth:.1%} revenue")
        
        if valuation_score >= 70:
            reasoning.append(f"Attractive valuation (P/E: {pe_ratio:.1f}, PEG: {peg_ratio:.1f})")
        ...
        return LongTermRating(
            ...
            distance_from_52w_high=(current_price - high_52w) / high_52w if high_52w else 0,
            reasoning=reasoning
        )
    
    def analyze_multiple(self, data: Dict[str, pd.DataFrame], info_dict: Dict[str, Dict]) -> List[LongTermRating]:
    ...
    def print_ratings(self, ratings: List[LongTermRating]):
        ...
        for rating in ratings:
            ...
            print(f"   ")
            print(f"   Key Metrics:")
            print(f"     P/E: {rating.pe_ratio:.1f} | PEG: {rating.peg_ratio:.1f} | D/E: {rating.debt_to_equity:.1f}")
            print(f"     Revenue Growth: {rating.revenue_growth:.1%} | EPS Growth: {rating.eps_growth:.1%}")
            print(f"     Price: ${rating.current_price:.2f} | 52W High: ${rating.price_52w_high:.2f}")
            print(f"   ")
            print(f"   Reasoning:")
