...
class MarketBreadthAnalyzer:
    ...
    def detect_divergence(
        ...
        breadth_history: List[BreadthData] = None
    ) -> Optional[str]:
        """
        ...
        index_return = (index_data['Close'].iloc[-1] - index_data['Close'].iloc[-5]) / index_data['Close'].iloc[-5]
    ...
    def calculate_market_health(
        ...
        if len(self.breadth_history) >= 5:
            ...
            old_adv = np.mean([b.advancing for b in self.breadth_history[-10:-5]]) if len(self.breadth_history) >= 10 else recent_adv
        ...
        # Detect divergence
        divergence = self.detect_divergence(index_data) if index_data is not None else None
    ...
    def print_breadth_report(
        ...
        if sector_breadth:
            ...
            for sector in sector_breadth[:5]:
                emoji = "🟢" if sector.strength > 0 else "🔴" if sector.strength < 0 else "🟡"
                print(f"  {emoji} {sector.sector:15} +{sector.advancing:3} / -{sector.declining:3} | "
...
def demo():
    ...
    # Define sectors
    sectors = {
        'Technology': ['AAPL', 'GOOGL', 'MSFT', 'NVDA', 'META', 'NFLX', 'AMD', 'INTC', 'CRM', 'ORCL'],
    ...
    sector_data = {sector: {sym: data[sym] for sym in stocks if sym in data} 
