...
class StockAnalyzer:
    ...
    def analyze_all(self) -> Dict[str, StockMetrics]:
        ...
        for symbol, df in self._data.items():
            try:
                ...
                print(f"✓ Analyzed {symbol}: {metrics.sharpe_ratio:.2f} Sharpe")
    ...
    def get_top_performers(self, metric: str = "sharpe_ratio", top_n: int = 10) -> List[StockMetrics]:
        """
...
def demo():
    ...
    for i, rec in enumerate(recs, 1):
        m = rec['metrics']
        print(f"  {i}. {m.symbol}: Score={rec['score']:.2f}, Risk={rec['risk_score']:.2f} → {rec['recommendation']}")
