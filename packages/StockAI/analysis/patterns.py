...
class PatternRecognizer:
    ...
    def _recognize_candlestick(self, symbol: str, df: pd.DataFrame):
        ...
        for i in range(1, len(df)):
            ...
            if (
                ...
                and row["Close"] > row["Open"]  # Current is green
                and row["Open"] < prev["Close"]  # Current open below prev close
                and row["Close"] > prev["Open"]
            ):  # Current close above prev open
            ...
            if (
                ...
                and upper_shadow < body * 0.5  # Small upper shadow
                and body > 0
            ):  # Has body
                self._patterns.append(
                    Pattern(
    ...
    def get_recent_patterns(self, days: int = 30) -> List[Pattern]:
        ...
        return [
            p for p in self._patterns if datetime.strptime(p.date, "%Y-%m-%d") > cutoff
        ]
...
def demo():
    ...
    for pattern in recent[:10]:  # Show first 10
        print(
            f"{pattern.date} | {pattern.type.value:20} | {pattern.signal:6} | {pattern.description}"
        )
