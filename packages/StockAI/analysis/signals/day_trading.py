...
class DayTradingAnalyzer:
    ...
    def detect_volume_spike(self, df: pd.DataFrame, threshold: float = 1.5) -> pd.Series:
        """Detect volume spikes (> threshold × average)."""
        avg_volume = df['Volume'].rolling(window=20).mean()
        volume_ratio = df['Volume'] / avg_volume
        return volume_ratio > threshold
    
    def mean_reversion_scan(self, df: pd.DataFrame, symbol: str) -> Optional[TradingSignal]:
        """
    ...
    def momentum_scan(self, df: pd.DataFrame, symbol: str) -> Optional[TradingSignal]:
        ...
        # Trend slope (5-day)
        if len(df) >= 5:
            trend_slope = (latest['Close'] - df.iloc[-5]['Close']) / df.iloc[-5]['Close']
        else:
            trend_slope = 0
    ...
    def gap_reversal_scan(self, df: pd.DataFrame, symbol: str) -> Optional[TradingSignal]:
        """
        ...
        if gap_percent > 0.04:  # Gap up >4%
            ...
            if day_change > 0.02:  # Reversing up
                signals.append(f"Gap down {gap_percent:.1%}, reversing {day_change:.1%}")
    ...
    def scan_multiple_stocks(self, data: Dict[str, pd.DataFrame]) -> Dict[str, List[TradingSignal]]:
    ...
    def get_top_signals(self, data: Dict[str, pd.DataFrame], top_n: int = 10) -> List[TradingSignal]:
    ...
    def print_signals(self, signals: List[TradingSignal]):
        ...
        for signal in signals:
            ...
            print(f"   Target: ${signal.target_price:.2f} ({(signal.target_price/signal.entry_price-1):+.1%})")
            print(f"   Stop: ${signal.stop_loss:.2f} ({(signal.stop_loss/signal.entry_price-1):+.1%})")
