...
class MarketScanner:
    ...
    def scan_biggest_movers(self, data: Dict[str, pd.DataFrame], top_n: int = 10) -> Dict:
        """
    ...
    def detect_volume_anomalies(self, data: Dict[str, pd.DataFrame], threshold: float = 2.0) -> List[MarketAnomaly]:
        """
        ...
        for symbol, df in data.items():
            ...
            if volume_ratio > threshold:
                ...
                signal = "BUY" if price_change > 0.02 else "SELL" if price_change < -0.02 else "WATCH"
    ...
    def detect_volatility_spikes(self, data: Dict[str, pd.DataFrame], threshold: float = 2.0) -> List[MarketAnomaly]:
        """
    ...
    def detect_sector_rotation(
        ...
        for sector, stocks in sector_data.items():
            ...
            for symbol, df in stocks.items():
                ...
                # 1-month return
                return_1m = (df['Close'].iloc[-1] - df['Close'].iloc[-21]) / df['Close'].iloc[-21]
    ...
    def detect_relative_strength(
        ...
        for symbol, df in data.items():
            ...
            stock_return = (df['Close'].iloc[-1] - df['Close'].iloc[-21]) / df['Close'].iloc[-21]
            
            # Compare to benchmark if available
            if benchmark is not None and len(benchmark) >= 21:
                benchmark_return = (benchmark['Close'].iloc[-1] - benchmark['Close'].iloc[-21]) / benchmark['Close'].iloc[-21]
            ...
            if relative_return > 0.10:  # Outperforming by >10%
                anomalies.append(MarketAnomaly(
                    ...
                    details={
                        ...
                        'benchmark_return': benchmark_return if benchmark is not None else None,
    ...
    def scan_all(self, data: Dict[str, pd.DataFrame], benchmark: pd.DataFrame = None) -> Dict:
        """
        Run all scans.
    ...
    def print_results(self, results: Dict):
        ...
        for gainer in results['movers']['gainers'][:5]:
            print(f"  {gainer['symbol']}: {gainer['change']:+.1%} @ ${gainer['price']:.2f}")
        
        print("\n📉 TOP LOSERS:")
        for loser in results['movers']['losers'][:5]:
            print(f"  {loser['symbol']}: {loser['change']:+.1%} @ ${loser['price']:.2f}")
...
class AlfredMarketScanner:
    ...
    def daily_scan(self, data: Dict[str, pd.DataFrame], benchmark: pd.DataFrame = None) -> Dict:
        """
    ...
    def _find_rebound_candidates(self, results: Dict) -> List[Dict]:
        ...
        for loser in results['movers']['losers']:
            ...
            volume_anomaly = next(
                (a for a in results['volume_anomalies'] if a.symbol == loser['symbol'] and a.signal == 'BUY'),
                None
            )
    ...
    def _find_momentum_trades(self, results: Dict) -> List[Dict]:
        ...
        for gainer in results['movers']['gainers']:
            # Check if there's a volume anomaly
            volume_anomaly = next(
                (a for a in results['volume_anomalies'] if a.symbol == gainer['symbol'] and gainer['change'] > 0.03),
                None
            )
    ...
    def print_daily_report(self, report: Dict):
        ...
        print(f"  Relative Strength Signals: {report['market_anomalies']['relative_strength']}")
